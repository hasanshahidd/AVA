...
import { integrationsApi, assetsApi } from '@/lib/api';
import {
  Loader2,
  Play,
  Square,
...
// Flow 1 — Ava drives the Nessus on your machine. Assets come from the IT
      ...
      <div className="shrink-0 bg-white rounded-xl border border-slate-200 p-3.5">
        <div className="flex flex-col lg:flex-row lg:items-center gap-3">
          {connLoading ? (
            ...
            <Link href="/scan-flows/connect" className="text-xs text-primary-600 font-medium shrink-0">Add a scan engine first →</Link>
            ...
            <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded-full border border-emerald-200 bg-emerald-50 text-xs text-emerald-700 shrink-0 self-start lg:self-auto">
              <Server size={12} /> Ava Engine <Check size={11} />
            </span>
          ) : (
            <select value={connId ?? ''} onChange={(e) => setConnId(e.target.value ? Number(e.target.value) : null)}
              ...
              {activeEngines.map((c, i) => <option key={c.id} value={c.id}>Ava Engine #{i + 1}</option>)}
            </select>
          )}
          ...
          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
            <div className="flex items-center gap-4 px-5 py-3 border-b border-slate-100">
              ...
              {shownActive ? (
                ...
                <Link href="/vulnerabilities" className="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-medium text-primary-700 bg-primary-50 border border-primary-200 rounded-lg hover:bg-primary-100 shrink-0">
                        ...
                        return (
                          <tr key={`${h.host}-${hi}`} className="hover:bg-slate-50/60">
                            <td className="px-5 py-2.5 font-mono text-slate-800 whitespace-nowrap">
                              {aid ? <Link href={`/assets/${aid}`} className="text-primary-700 hover:underline inline-flex items-center gap-1">{h.host}<ExternalLink size={11} /></Link> : h.host}
                            </td>
              ...
              {connId != null && (
                <div>
                  ...
                  {credLoading ? (
                    ...
                    <span className="text-xs text-slate-400">none saved — <Link href="/scan-flows/connect" className="text-primary-600 font-medium">add one</Link> for an authenticated scan</span>
                  ) : (
