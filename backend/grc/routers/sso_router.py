...
A single Ava-owned Azure app registration (env: ENTRA_CLIENT_ID /
...
# /auth/entra so the redirect URI configured in the Ava Azure app
# (https://ava.ai/api/auth/entra/callback) lines up.
...
def _entra_app_credentials() -> Tuple[str, str]:
    """Look up the Ava-owned Azure app credentials. Multiple names
    are accepted so existing deployments don't have to rename their env vars."""
    cid = _env_first(
        "ENTRA_CLIENT_ID",
        "ava_client_id", "AVA_CLIENT_ID",
        "APPLICATION_CLIENT_ID",
        "AZURE_CLIENT_ID",
    )
    csec = _env_first(
        "ENTRA_CLIENT_SECRET",
        "ava_client_secret", "AVA_CLIENT_SECRET",
    ...
    if not cid or not csec:
        raise HTTPException(
            ...
            detail=(
                ...
                "ava_client_id / ava_client_secret — in the backend env)."
            ),
        )
    return cid, csec
...
def _redirect_uri(request: Request) -> str:
    ...
    EXACTLY one of the redirect URIs registered on the Ava Azure app.

    Returns an *apex* URL (subdomain stripped) so a single redirect URI in
    Azure works for every Ava tenant. The state JWT carries the
...
def _attach_state_cookie(response, state: str, request: Optional[Request] = None) -> None:
    ...
    (e.g. `.ava.ai`); for dev `*.localhost` is auto-detected."""
