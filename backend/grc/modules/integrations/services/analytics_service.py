...
class AnalyticsService:
    ...
    def get_overview(db: Session, tenant_id: int, connection_id: Optional[int] = None) -> Dict[str, Any]:
        ...
        severity_counts = dict(
            db.query(
                Vulnerability.ava_severity,
                func.count(Vulnerability.id),
            ).filter(*base_filter).group_by(Vulnerability.ava_severity).all()
        )
    ...
    def get_scoring_distribution(
        ...
        base_filter = [
            Vulnerability.tenant_id == tenant_id,
            Vulnerability.source.in_(SCANNER_SOURCES),
            Vulnerability.ava_risk_score.isnot(None),
        ]
        if connection_id:
            base_filter.append(Vulnerability.connection_id == connection_id)

        avg_score = db.query(func.avg(Vulnerability.ava_risk_score)).filter(*base_filter).scalar()
        max_score = db.query(func.max(Vulnerability.ava_risk_score)).filter(*base_filter).scalar()
        min_score = db.query(func.min(Vulnerability.ava_risk_score)).filter(*base_filter).scalar()
