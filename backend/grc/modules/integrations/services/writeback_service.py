...
When a finding is decided in Ava — false positive, risk accepted,
...
class WritebackService:
            ...
            try:
                ...
                if not enabled:
                    ...
                    row.skip_reason = (
                        ...
                        "(provider_config.scanner_writeback). Decision recorded in Ava only."
                    )
                    row.updated_at = now
                    counts["skipped"] += 1
    ...
    def _process_push(
        ...
        payload: Dict[str, Any] = {
            "reason": row.action,
            "comment": f"Ava decision: {row.action} (vuln {vuln.vuln_id}, trigger: {row.trigger})",
