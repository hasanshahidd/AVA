...
def _nessus_live_detail(db: Session, run: HostedScanRun, tenant_id: int) -> Optional[dict]:
    ...
    /scans/{id} on each poll, so the Ava panel mirrors what Nessus itself shows
...
def trigger_hosted_scan(
    ...
    run = HostedScanRun(
        ...
        scan_name=body.scan_name or "Ava hosted scan",
...
def delete_hosted_scan(
    ...
    """Delete a scan session from history. Removes the Ava session row only —
