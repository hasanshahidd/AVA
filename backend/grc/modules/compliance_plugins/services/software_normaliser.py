...
def _is_runnable_check(check_definition) -> bool:
    ...
    if isinstance(expect, dict) and expect.get("kind") == "any":
        return False
    # Dangling-pipe commands (`grep ... |` with nothing after the final pipe)
    # always error, so they are not runnable. Mirror of the SQL guard in
    # strict_matcher.runnable_check_clauses() — keep the two in step.
    cmd = check_definition.get("command")
    if isinstance(cmd, str) and re.search(r"\|\s*$", cmd):
        return False
    return True
