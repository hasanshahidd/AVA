...
def match_preview(
    ...
    Read-only: does NOT execute scans. Ava is not a scanner —
...
async def ingest_cis_pdf(
    ...
    # Ava ingests them centrally. Anyone not in
    # AVA_PLATFORM_ADMINS (.env) gets a 403.
    current_user: GRCUser = Depends(require_platform_admin),
):
...
def review_plugins_bulk(
    ...
    Differentiator vs Cywift: Cywift requires one-by-one approval; Ava
...
def delete_benchmark_mapping(
    ...
    if not m:
        raise HTTPException(404, "Mapping not found (or it's a global mapping — only Ava can delete those)")
    db.delete(m)
    db.commit()
...
def _do_scan_all(
    ...
    *, benchmark, runner_type, connection_id,
    ...
    from .services.strict_matcher import runnable_check_clauses
    q = db.query(CompliancePlugin).filter(
        (CompliancePlugin.tenant_id.is_(None)) | (CompliancePlugin.tenant_id == tenant_id),
        CompliancePlugin.enabled.is_(True),
        ...
        CompliancePlugin.review_status.in_(["approved", "auto_approved"]),
        # Well-formed-check exclusions (TODO / kind:any / dangling-pipe). This
        # worker builds its own query rather than calling
        # applicable_plugins_for_asset(), so it must splice in the SAME shared
        # clause set — the drift this centralises is exactly what once let a
        # TODO rule reach the scan path and record a bogus security failure.
    ...
    max_workers = int(os.environ.get("AVA_SCAN_CONCURRENCY", "10"))
    if _work_queue:
