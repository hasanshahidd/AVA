...
def egress_ips(
    ...
    """Return the public IPs / FQDNs Ava uses to call into the
    ...
    The list is configurable per-region via the `AVA_EGRESS_IPS`
    ...
    """
    import os
    raw = os.environ.get("AVA_EGRESS_IPS", "").strip()
    region = os.environ.get("AVA_REGION", "default")
    ...
    if raw:
        ...
        return {
            ...
            "note": (
                ...
                "ports. Ava will scan from these addresses only."
            ),
        }
    ...
    return {
        ...
        "configured": False,
        "note": (
            "AVA_EGRESS_IPS env var is not configured on this deploy. "
...
class ADDiscoverIn(BaseModel):
    ...
    bind_dn: str = Field(..., description="CN=svc-ava,OU=Service Accounts,DC=bank,DC=local — or DOMAIN\\\\user for NTLM")
